...
系统用 Chroma 管理长期知识，用配置化数据库 backend 管理情景记忆，用 Pydantic 做结构化约束，用 workflow trace 做可观测性。
...
```text
短期记忆是一次请求里的 workflow state；长期记忆是 Chroma 知识库；情景记忆是数据库 backend 中的生成历史。默认 SQLite 便于本机开发，MySQL backend 用于后续多实例共享状态。三者分开后，既能控制上下文长度，也能支持回放、评估和持续优化。
```
...
封版后的升级应以 [architecture-baseline.md](architecture-baseline.md) 作为对照基线，以 [release-checklist.md](release-checklist.md) 作为交付检查清单。后续引入 Redis 队列、MySQL 或 LangGraph 时，应优先保持现有 API 语义、状态字段和 workflow trace 可观测输出稳定。
