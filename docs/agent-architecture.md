...
```text
需求输入 -> 需求分析 -> 知识检索 -> 测试策略规划 -> Prompt 构造 -> LLM 生成 -> Schema 校验 -> 后处理 -> Reviewer 审查 -> 条件路由 -> 质量评分/历史记录
```
...
`route_after_review` 是当前第一条显式条件边：
...
### 3.5 Reviewer 为什么用本地规则而不是再调一次大模型
...
- 增加更多条件边：检索不足 -> query rewrite；成本超限 -> 人工确认；质量低且多次失败 -> 人工审核。
