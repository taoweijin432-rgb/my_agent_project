...
| P1 | 线上不可观测 | 有 readiness/queue check、内部 metrics、告警模板、示例配置和离线验证，无正式采集/告警 | 下一阶段优先接入真实 Prometheus/Alertmanager 并校准阈值 |
| P1 | 真实模型质量随模型切换漂移 | 已有真实 LLM strict eval 和 benchmark 入口，样本仍需扩展 | 模型切换后必须跑真实 strict eval，并记录耗时/限流趋势 |
