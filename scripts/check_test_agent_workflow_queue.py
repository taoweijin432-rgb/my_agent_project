...
class TestAgentWorkflowJobCounter(Protocol):
...
def build_database_snapshot(
    settings: Settings,
    store: TestAgentWorkflowJobCounter | None = None,
) -> dict[str, Any]:
    job_store = store or create_test_agent_workflow_job_store(settings)
...
def evaluate_health(database: dict[str, Any], queue: dict[str, Any]) -> QueueHealth:
    ...
    if queue.get("backend") != "rq" or not queue.get("active", True):
        if database_active:
            warnings.append(
                "Database has active test agent workflow jobs while Redis/RQ "
    ...
    if failed_count:
        warnings.append(
            f"RQ failed registry contains {failed_count} test agent workflow job(s)."
        )

    if database_active > rq_active:
        errors.append(
            "Database active test agent workflow jobs exceed Redis/RQ active "
            f"test agent workflow jobs (database={database_active}, rq={rq_active})."
        )
    if database_active == 0 and rq_active > 0:
        errors.append(
            "Redis/RQ has active test agent workflow jobs but the database has no "
            f"queued/running test agent workflow jobs (rq={rq_active})."
        )
    if counts.get("running", 0) and int(queue.get("started") or 0) == 0:
        warnings.append(
            "Database has running test agent workflow jobs but RQ started registry "
            "is empty; check worker heartbeat and stale job recovery."
        )
    if rq_active and int(queue.get("worker_count") or 0) == 0:
        warnings.append(
            "Redis/RQ has active test agent workflow jobs but no live worker was found."
        )
...
def print_text(snapshot: dict[str, Any]) -> None:
    ...
    print("Test agent workflow queue snapshot")
    print("Database")
    print(f"  backend: {database['backend']}")
    print(f"  configured_database_backend: {database['configured_database_backend']}")
    ...
    if queue.get("backend") == "rq":
        ...
        print(f"  function: {queue['function']}")
        if queue.get("error"):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Inspect test agent workflow queue health."
    )
    ...
    try:
        ...
        if args.json:
            ...
            print("Test agent workflow queue check failed")
