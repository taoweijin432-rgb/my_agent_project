...
class TestPlanExecutionJobCounter(Protocol):
...
def build_database_snapshot(
    settings: Settings,
    store: TestPlanExecutionJobCounter | None = None,
) -> dict[str, Any]:
    job_store = store or create_test_plan_execution_job_store(settings)
...
def evaluate_health(database: dict[str, Any], queue: dict[str, Any]) -> QueueHealth:
    ...
    if queue.get("backend") != "rq" or not queue.get("active", True):
        if database_active:
            warnings.append(
                "Database has active test plan execution jobs while Redis/RQ "
    ...
    if failed_count:
        warnings.append(
            f"RQ failed registry contains {failed_count} test plan execution job(s)."
        )

    if database_active > rq_active:
        errors.append(
            "Database active test plan execution jobs exceed Redis/RQ active "
            f"test plan execution jobs (database={database_active}, rq={rq_active})."
        )
    if database_active == 0 and rq_active > 0:
        errors.append(
            "Redis/RQ has active test plan execution jobs but the database has no "
            f"queued/running test plan execution jobs (rq={rq_active})."
        )
    if counts.get("running", 0) and int(queue.get("started") or 0) == 0:
        warnings.append(
            "Database has running test plan execution jobs but RQ started registry "
            "is empty; check worker heartbeat and stale job recovery."
        )
    if rq_active and int(queue.get("worker_count") or 0) == 0:
        warnings.append(
            "Redis/RQ has active test plan execution jobs but no live worker was found."
        )
...
def print_text(snapshot: dict[str, Any]) -> None:
    ...
    print("Test plan execution queue snapshot")
    print("Database")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Inspect test plan execution queue health."
    )
    ...
    try:
        ...
        if args.json:
            ...
            print("Test plan execution queue check failed")
